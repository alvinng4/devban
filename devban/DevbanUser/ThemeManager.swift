...
extension DevbanUser
{
    ...
    /// A centralized manager for handling custom themes throughout the application. It is a subclass of `DevbanUser`.
    ///
    /// The `ThemeManager` provides a instance that manages theme-related properties
    /// such as background colors and button colors, with support for light and dark mode.
    ///
    /// ## Usage
    /// ```swift
    /// // Access theme colors
    /// let backgroundColor: Color = themeManager.backgroundColor
    /// let buttonColor: Color = themeManager.buttonColor
    ///
    /// // Update theme for color scheme changes
    /// themeManager.updateTheme(theme: theme, colorScheme: .dark)
    /// ```
    @Observable
    final class ThemeManager
    {
        @MainActor
        init(theme: DefaultTheme, colorScheme: ColorScheme)
        {
        ...
        @MainActor
        static func getBackgroundColor(theme: DefaultTheme, colorScheme: ColorScheme) -> Color
        {
            return (
                colorScheme == .dark,
        ...
        @MainActor
        static func getBackgroundColorLight(theme: DefaultTheme) -> Color
        {
            switch (theme)
            {
                case .blue:
        ...
        @MainActor
        static func getBackgroundColorDark(theme: DefaultTheme) -> Color
        {
            switch (theme)
            {
                case .blue:
        ...
        @MainActor
        static func getButtonColor(theme: DefaultTheme) -> Color
        {
            switch (theme)
            {
                case .blue:
