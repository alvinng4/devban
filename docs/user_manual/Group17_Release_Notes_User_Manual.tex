...
\textbf{Version}: 1.0.0 | \textbf{Platform}: iOS / iPadOS / macOS 26.0 or later | \textbf{Release Date}: \today

\section{Introduction}

\textbf{Devban} is a collaborative task management application designed specifically
for development teams. This initial \linebreak release (v1.0.0) combines Kanban-style task
organization, calendar scheduling, AI-powered assistance, and \linebreak gamification elements
...
(Ideally the app should be distributed via App Store, but due to many issues it is not possible for this project.)
\begin{enumerate}
...
\subsection{Login}
Simply follow the graphical interface to login.
...
To create a new team, contact us via \url{csci3100group17@gmail.com} to receive a License Key. Then, follow the instructions on the
...
To join an existing team, ask your admin to obtain an invite code, which will expire in 7 days.
...
\begin{itemize}
    ...
    \item Unlimited rate with Apple Intelligence model (However, this model is not supported on some devices)
\end{itemize}
